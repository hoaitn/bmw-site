(function(c) {
	...
	var b = {
		...
		_init : function() {
			...
			r.isIE = !r.isWebKit && !r.isOpera && (/MSIE/gi).test(f)
					&& (/Explorer/gi).test(m.appName);
			...
			function g(i) {
				if (i.src
						&& /tiny_mce(|_gzip|_jquery|_prototype)(_dev|_src)?.js/
								.test(i.src)) {
...
(function() {
	function a(g) {
		...
		this.addRange = function(l) {
			var t, A, z = g.dom.doc, r = z.body, u, n, y, o, s, k, p, q, x, m;
			this.destroy();
			y = l.startContainer;
			o = l.startOffset;
			s = l.endContainer;
			...
			if (s.nodeType == 1 && s.hasChildNodes()) {
				q = s.childNodes.length - 1;
				if (k == 0) {
					m = 1;
					s = s.childNodes[0]
				} else {
					s = s.childNodes[Math.min(q, k - 1)];
					if (s.nodeType == 3) {
						k = s.nodeValue.length
					}
				}
			}
			if (y == s && y.nodeType == 1) {
				if (/^(IMG|TABLE)$/.test(y.nodeName) && o != k) {
					t = r.createControlRange();
					t.addElement(y)
				} else {
					t = r.createTextRange();
				...
				if (o == k) {
					t.collapse(k <= l.endContainer.childNodes.length - 1)
				}
				t.select();
				t.scrollIntoView();
				return
			...
			if (s.nodeType == 3) {
				...
				A.moveToElementText(p);
				p.parentNode.removeChild(p);
				A.move("character", k);
				t.setEndPoint("EndToStart", A)
			} else {
				A.moveToElementText(s);
				A.collapse(!!m);
				t.setEndPoint("EndToEnd", A)
			}
			t.select();
			t.scrollIntoView()
		};
...
(function() {
	...
	var b = function(E, t, B, v) {
		...
		var C = [], D, z, H, G, A, s, r = true, x = o(t);
		p.lastIndex = 0;
		...
		if (C.length > 1 && j.exec(E)) {
			...
			if (!v && C.length > 1 && t.nodeType === 9 && !x
					&& f.match.ID.test(C[0])
	...
	b.filter = function(B, A, E, t) {
		var s = B, G = [], y = A, v, e, x = A && A[0] && o(A[0]);
		while (B && A.length) {
			for ( var z in f.filter) {
				if ((v = f.match[z].exec(B)) != null) {
					var r = f.filter[z], F, D;
					e = false;
					if (y == G) {
						G = []
					}
	...
	var f = b.selectors = {
		...
		relative : {
			"+" : function(y, e, x) {
				var u = typeof e === "string", z = u && !/\W/.test(e), v = u
						&& !z;
				if (z && !x) {
					e = e.toUpperCase()
				}
			...
			.create(
					"tinymce.dom.Selection",
					{
						...
						moveToBookmark : function(m) {
							var q = this, l = q.dom, i, h, f, p, j, r, n, o;
							if (q.tridentSel) {
								q.tridentSel.destroy()
							}
							if (m) {
								if (m.start) {
									f = l.createRng();
									p = l.getRoot();
									function g(x) {
										var s = m[x ? "start" : "end"], t, u, v;
										if (s) {
											for (u = p, t = s.length - 1; t >= 1; t--) {
												u = u.childNodes[s[t]]
											}
											if (x) {
												f.setStart(u, s[0])
									...
									if (m.id) {
										function k(z) {
											var t = l.get(m.id + "_" + z), y, s, v, x, u = m.keep;
											if (t) {
												y = t.parentNode;
												if (z == "start") {
													if (!u) {
												...
												if (!u) {
													...
													while (t = l.get(m.id + "_"
															+ z)) {
														l.remove(t, 1)
													}
													if (x
															&& v
															&& x.nodeType == v.nodeType
															&& x.nodeType == 3) {
														s = x.nodeValue.length;
														x
																.appendData(v.nodeValue);
														l.remove(v);
														if (z == "start") {
															j = r = x;
															n = o = s
										...
										if (m.name) {
											q.select(l.select(m.name)[m.index])
										} else {
											if (m.rng) {
												q.setRng(m.rng)
											}
						...
						getRng : function(j) {
							...
							if (!i) {
								i = g.win.document.createRange ? g.win.document
										.createRange() : g.win.document.body
										.createTextRange()
							}
					...
					"tinymce.dom.Serializer",
					{
						Serializer : function(j) {
							...
							if (j.fix_table_elements) {
								i.onPreProcess.add(function(k, l) {
									if (!e.isOpera
											|| opera.buildNumber() >= 1767) {
										f(i.dom.select("p table", l.node)
												.reverse(), function(p) {
											var o = i.dom.getParent(
													p.parentNode, "table,p");
											if (o.nodeName != "TABLE") {
												try {
													i.dom.split(o, p)
												} catch (m) {
						...
						serialize : function(r, q) {
							...
							if (j.createHTMLDocument
									&& (e.isOpera && opera.buildNumber() >= 1767)) {
						...
						_serializeNode : function(D, I) {
							var z = this, A = z.settings, x = z.writer, q, j, u, F, E, H, B, h, y, k, r, C, p, m, G, o;
							if (!A.node_filter || A.node_filter(D)) {
								switch (D.nodeType) {
								case 1:
									if (D.hasAttribute ? D
											.hasAttribute("_mce_bogus") : D
											.getAttribute("_mce_bogus")) {
										return
									}
									p = G = false;
									q = D.hasChildNodes();
									k = D.getAttribute("_mce_name")
									...
									if (!G) {
										if (!z.validElementsRE
												|| !z.validElementsRE.test(k)
												|| (z.invalidElementsRE && z.invalidElementsRE
														.test(k)) || I) {
											p = true;
											break
										}
									...
									if (k === "script" && e.trim(D.innerHTML)) {
										x.writeText("// ");
										x.writeCDATA(D.innerHTML.replace(
												/<!--|-->|<\[CDATA\[|\]\]>/g,
												""));
										q = false;
										break
								...
								case 3:
									if (A.validate_children
											&& z.elementName
											&& !z.schema.isValid(z.elementName,
													"#text")) {
										return
									}
						...
						_addEvents : function() {
							...
							i(
									...
									function(t, s) {
										...
										case "reset":
											...
											break;
										default:
											v.dom.bind(y.content_editable ? v
													.getBody() : v.getDoc(), s,
													r)
										}
									});
							v.dom.bind(y.content_editable ? v.getBody()
									: (a ? v.getDoc() : v.getWin()), "focus",
									function(s) {
										v.focus(true)
									});
							if (m.isGecko) {
								...
								v.onClick.add(function(s, t) {
									...
									if (t.nodeName == "IMG"
											|| (t.nodeName == "A" && v.dom
													.hasClass(t,
															"mceItemAnchor"))) {
										v.selection.getSel().setBaseAndExtent(
												t, 0, t, 1)
									}
								})
							}
							...
							if (y.custom_shortcuts) {
								if (y.custom_undo_redo_keyboard_shortcuts) {
									v.addShortcut("ctrl+z", v
											.getLang("undo_desc"), "Undo");
									v.addShortcut("ctrl+y", v
											.getLang("redo_desc"), "Redo")
								}
								v.addShortcut("ctrl+b", v.getLang("bold_desc"),
										"Bold");
								v.addShortcut("ctrl+i", v
										.getLang("italic_desc"), "Italic");
								v
										.addShortcut("ctrl+u", v
												.getLang("underline_desc"),
												"Underline");
								for (u = 1; u <= 6; u++) {
									v.addShortcut("ctrl+" + u, "", [
											"FormatBlock", false, "h" + u ])
								}
								v.addShortcut("ctrl+7", "", [ "FormatBlock",
										false, "<p>" ]);
								v.addShortcut("ctrl+8", "", [ "FormatBlock",
										false, "<div>" ]);
								v.addShortcut("ctrl+9", "", [ "FormatBlock",
										false, "<address>" ]);
								function z(t) {
									var s = null;
								...
								v.onKeyDown.add(function(s, t) {
									...
									case 8:
										if (v.selection.getRng().item) {
											s.dom.remove(v.selection.getRng()
													.item(0));
											return j.cancel(t)
										}
									}
...
(function(l) {
	var k = l.dom.Event, c = l.isIE, a = l.isGecko, b = l.isOpera, j = l.each, i = l.extend, d = true, h = false;
	function g(n, o) {
			...
			.create(
					"tinymce.ForceBlocks",
					{
						ForceBlocks : function(n) {
							...
							o.reOpera = new RegExp("(\\u00a0|&#160;|&nbsp;)</"
									+ q + ">", "gi");
							o.rePadd = new RegExp(
									"<p( )([^>]+)><\\/p>|<p( )([^>]+)\\/>|<p( )([^>]+)>\\s+<\\/p>|<p><\\/p>|<p\\/>|<p>\\s+<\\/p>"
											.replace(/p/g, q), "gi");
							o.reNbsp2BR1 = new RegExp(
									"<p( )([^>]+)>[\\s\\u00a0]+<\\/p>|<p>[\\s\\u00a0]+<\\/p>"
											.replace(/p/g, q), "gi");
							o.reNbsp2BR2 = new RegExp(
									"<%p()([^>]+)>(&nbsp;|&#160;)<\\/%p>|<%p>(&nbsp;|&#160;)<\\/%p>"
											.replace(/%p/g, q), "gi");
							o.reBR2Nbsp = new RegExp(
									"<p( )([^>]+)>\\s*<br \\/>\\s*<\\/p>|<p>\\s*<br \\/>\\s*<\\/p>"
											.replace(/p/g, q), "gi");
							function m(r, s) {
								if (b) {
									s.content = s.content.replace(o.reOpera,
											"</" + q + ">")
								}
								s.content = s.content.replace(o.rePadd, "<" + q
										+ "$1$2$3$4$5$6>\u00a0</" + q + ">");
								if (!c && !b && s.set) {
									s.content = s.content
											.replace(o.reNbsp2BR1, "<" + q
													+ "$1$2><br /></" + q + ">");
									s.content = s.content
											.replace(o.reNbsp2BR2, "<" + q
													+ "$1$2><br /></" + q + ">")
								} else {
									s.content = s.content
											.replace(o.reBR2Nbsp, "<" + q
													+ "$1$2>\u00a0</" + q + ">")
								}
							}
						...
						setup : function() {
							...
							if (l.isWebKit) {
								function q(t) {
									var s = o.getRng(), u, y = r.create("div",
											null, " "), x, v = r.getViewPort(t
											.getWin()).h;
								...
								m.onKeyPress
										.add(function(s, t) {
											if (t.keyCode == 13
													&& (t.shiftKey || (p.force_br_newlines && !r
															.getParent(o
																	.getNode(),
						...
						find : function(u, p, q) {
							var o = this.editor, m = o.getDoc()
									.createTreeWalker(u, 4, null, h), r = -1;
							while (u = m.nextNode()) {
								r++;
								if (p == 0 && u == q) {
									return r
						...
						forceRoots : function(v, H) {
							var y = this, v = y.editor, L = v.getBody(), I = v
									.getDoc(), O = v.selection, z = O.getSel(), A = O
									.getRng(), M = -2, u, F, m, o, J = -16777215;
							var K, p, N, E, B, q = L.childNodes, D, C, x;
							for (D = q.length - 1; D >= 0; D--) {
								K = q[D];
								if (K.nodeType === 1
										&& K.getAttribute("_mce_type")) {
									p = null;
									continue
								}
								if (K.nodeType === 3
										|| (!y.dom.isBlock(K)
												&& K.nodeType !== 8 && !/^(script|mce:script|style|mce:style)$/i
												.test(K.nodeName))) {
									if (!p) {
										if (K.nodeType != 3
												|| /[^\s]/g.test(K.nodeValue)) {
											if (M == -2 && A) {
												if (!c) {
													if (A.startContainer.nodeType == 1
															&& (C = A.startContainer.childNodes[A.startOffset])
															&& C.nodeType == 1) {
														x = C
																.getAttribute("id");
														C.setAttribute("id",
													...
													o.collapse(1);
													N = o.move("character", J)
															* -1;
													o = A.duplicate();
													o.collapse(1);
													E = o.move("character", J)
															* -1;
													o = A.duplicate();
													o.collapse(0);
													B = (o.move("character", J) * -1)
															- E;
													M = E - N;
													u = B
												}
											...
											p = v.dom
													.create(v.settings.forced_root_block);
							...
							if (M != -2) {
								if (!c) {
									p = L
											.getElementsByTagName(v.settings.element)[0];
									A = I.createRange();
									if (M != -1) {
						...
						insertPara : function(R) {
							var F = this, v = F.editor, N = v.dom, S = v
									.getDoc(), W = v.settings, G = v.selection
									.getSel(), H = G.getRangeAt(0), V = S.body;
							var K, L, I, P, O, q, o, u, z, m, D, U, p, x, J, M = N
									.getViewPort(v.getWin()), C, E, B;
							K = S.createRange();
							...
							if (P == V && q == V && V.firstChild
									&& v.dom.isBlock(V.firstChild)) {
								P = q = P.firstChild;
								O = o = 0;
								K = S.createRange();
								K.setStart(P, 0);
							...
							if (/(TD|TABLE|TH|CAPTION)/.test(m)
									|| (u && m == "DIV" && /left|right/gi
											.test(N.getStyle(u, "float", 1)))) {
								m = W.element;
								u = z = null
							}
							D = (u && u.nodeName == m) ? u.cloneNode(0) : v.dom
									.create(m);
							U = (z && z.nodeName == m) ? z.cloneNode(0) : v.dom
									.create(m);
							U.removeAttribute("id");
							...
							do {
								...
								p = J
							} while ((J = J.previousSibling ? J.previousSibling
									: J.parentNode));
							J = x = q;
							do {
								if (J == V
								...
								x = J
							} while ((J = J.nextSibling ? J.nextSibling
									: J.parentNode));
							if (p.nodeName == m) {
								K.setStart(p, 0)
							} else {
							...
							if (!p.previousSibling
								...
								if (K.startContainer.nodeName == m
										&& K.startOffset == 0) {
						...
						backspaceDelete : function(s, A) {
							var C = this, q = C.editor, x = q.getBody(), p = q.dom, o, u = q.selection, m = u
									.getRng(), v = m.startContainer, o, y, z;
							if (v && q.dom.isBlock(v)
									&& !/^(TD|TH)$/.test(v.nodeName) && A) {
								if (v.childNodes.length == 0
										|| (v.childNodes.length == 1 && v.firstChild.nodeName == "BR")) {
									o = v;
									...
									if (o) {
										if (v != x.firstChild) {
											y = q.dom.doc.createTreeWalker(o,
											...
											m
													.setStart(
															o,
															o.nodeValue ? o.nodeValue.length
																	: 0);
											m
													.setEnd(
															o,
...
(function(a) {
	a.Formatter = function(T) {
		...
		function z(W, ae, Y) {
			...
			function aa(ai) {
				var aj;
				M(l(ai.parentNode).reverse(), function(ak) {
					var al;
			...
			function ab(ak) {
				var aj = c.get(ak ? "_start" : "_end"), ai = aj[ak ? "firstChild"
						: "lastChild"];
				if (G(ai)) {
					ai = ai[ak ? "firstChild" : "lastChild"]
				}
				c.remove(aj, true);
				return ai
			}
					...
					M("onKeyDown,onKeyUp,onKeyPress,onMouseUp".split(","),
							function(ad) {
								T[ad].addToTop(function(ae, af) {
									if (ab()
											&& !a.dom.RangeUtils.compareRanges(
													N.lastRng, p.getRng())) {
